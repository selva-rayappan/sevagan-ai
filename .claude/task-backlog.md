...
| [Phase 14](#phase-14--technician-job-offer-voice-escalation) | Technician Job-Offer Voice Escalation | ✅ COMPLETE | 40/42 (job offers now deliver outside the 24h WhatsApp session via approved templates, validated live 2026-08-19; escalation-call `answer_method` switched GET→POST after intermittent "Invalid Answer XML" hangups — unconfirmed pending the next live call; Plivo HMAC-V3 signature and a carrier-level DND issue on one technician's number remain documented non-blocking gaps) |
...
| 14.8.3 | `VoiceWebhookController`: XML-building extracted into `buildAnswerXml(lang)`, shared by GET (kept for manual testing, no longer hit by Plivo) and POST. Plivo POSTs to the *same* `/voice/answer` URL for two purposes — initial answer fetch, and its default post-hangup status callback (no `hangup_url` configured) — distinguished by the `Event` field, present only on the hangup callback; the initial-answer POST now serves the real `<GetDigits>`/`<Play>` XML instead of always acking empty | ✅ |
| 14.8.4 | Unit tests: `plivo-voice-call.provider.spec.ts` (`answer_method: 'POST'`), `voice-webhook.controller.spec.ts` (hangup callback with `Event` acks empty; initial POST with no `Event` serves real XML per language; logs the request) | ✅ |
| 14.8.5 | Full backend suite green (73 suites / 621 tests), `tsc --noEmit` clean, coverage 96.83%/88%/92.05%/97.21%, `voice-webhook.controller.ts` 100% all four metrics | ✅ |
| 14.8.6 | Confirm POST actually resolves the intermittent failure on a real call | ❌ Unconfirmed — watching the next live escalation call |
